...
Screen.height / 2 - 20, Screen.width / 6 );
	...
	void Update () {
		...
		if (!paused && !scout) {
			...
			if (nextPressed) {
				...
				PlayerPrefs.SetInt(WelcomeMenuScene.SCORE_KEY, statsUi.getScore()); // save score for next level, carry it on
				Application.LoadLevel(currentLevel);
			} else if (backPressed) {
				Time.timeScale = 1;
				Application.LoadLevel(0);
	...
	void Start(){
		...
		if (PlayerPrefs.GetInt(WelcomeMenuScene.GAME_MODE) == 0) {
			storyMode = true;
		} else {
			endlessMode = true;
		}
		if (PlayerPrefs.GetInt(WelcomeMenuScene.SCORE_KEY) != 0) { // carry on score from previous level
			statsUi.setScore(PlayerPrefs.GetInt(WelcomeMenuScene.SCORE_KEY));
		}
	}
	void FixedUpdate() {
	...
	void die() {
		...
		PlayerPrefs.SetInt(WelcomeMenuScene.SCORE_KEY, 0); // reset saved score
		Time.timeScale = 0;
	}
