...
public class TorizoScript : MonoBehaviour {
	...
	void FixedUpdate() {
		...
		if (Mathf.Abs(playerScript.transform.position.x - transform.position.x) >= 70f || Mathf.Abs(transform.position.y - playerScript.transform.position.y) >= 40f) {
			...
			if (startAnim) {
				...
				if (animTimer <= 0.2f && !instantiated) {
					Instantiate (DamagingPrefab, firePoint.position, firePoint.rotation);
					attackCounter++;
					instantiated = true;
	...
	void OnCollisionEnter2D(Collision2D collision) {
		if (collision.collider.name.StartsWith("Missile")) {
			...
			if (health <= 0) {
				...
				playerScript.GainScore(300);
				if(controlUI.getCurrentLevel() == 4){
					door.OpenDoor();
				}
	...
	void OnGUI() {
		...
		int currHP = GameObject.Find (gameObject.name).GetComponent<TorizoScript>().getHealth();
		for (int i = 0; i < 10; i++) {
			if (i * 40 < currHP) {
